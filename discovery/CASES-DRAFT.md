...
The corpus (42 lines: header + 41 data rows) lives in [`fixtures/corpus.csv`](fixtures/corpus.csv).  
Run `python discovery/cli.py init --product test --input discovery/fixtures/corpus.csv` to exercise any scenario, or `python discovery/cli.py demo` for the full d1→d4 run with baked d3 labels (no LLM).
...
The corpus is stored as a real file at [`discovery/fixtures/corpus.csv`](fixtures/corpus.csv) — it is no longer embedded inline in this document. Header row plus 41 data rows. Edit the file there if a scenario needs a new pathological input; do not re-embed the CSV here.

**Corpus breakdown (41 data rows):**
